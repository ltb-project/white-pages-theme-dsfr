...
{if $type eq 'text'}
    <input type="text" name="{$item}{$itemindex}" class="fr-input" value="{$value}" data-role="value" />
...
{else if $type eq 'date'}
    <input type="date" class="fr-input" name="{$item}{$itemindex}" value="{convert_ldap_date($value)|date_format:"%Y-%m-%d"}" data-role="value" />
{else if $type eq 'ad_date'}
    <input type="date" class="fr-input" name="{$item}{$itemindex}" value="{convert_ad_date($value)|date_format:"%Y-%m-%d"}" data-role="value" />
{else if $type eq 'static_list' or $type eq 'list'}
    <select class="fr-select" name="{$item}{$itemindex}" data-role="value">
        <option></option>
...
{/if}
  ...
  {if $multivalued == "true" and $itemindex == 0}
    <button type="button" class="fr-btn fr-icon-add-circle-line" data-action="add" data-item="{$item}">add</button>
  {/if}
  {if $multivalued == "true" and $itemindex > 0}
    <button type="button" class="fr-btn fr-btn--secondary fr-icon-delete-line" data-action="del" data-item="{$item}" data-index="{$itemindex}">del</button>
  {/if}
  </div>
</div>
